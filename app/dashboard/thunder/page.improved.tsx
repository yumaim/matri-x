  ...
  return (
    <div className="rounded-xl border border-border bg-card/50 p-6">
      <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
        ...
        <TechTerm
          term="Kafka"
          definition="リアルタイムでイベントを配信するメッセージングシステム"
          example="LINEの通知のように、投稿や削除の情報が瞬時に届く仕組み"
        />
        {" "}ストリーム シミュレーション
      </h3>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          <div className="text-sm font-medium text-muted-foreground mb-2 flex items-center gap-1.5">
            ...
            <TechTerm
              term="インメモリストア"
              definition="データをコンピュータのメインメモリ(RAM)に保存する仕組み"
              example="HDDではなくRAMに保存するので、SSDより100倍以上高速"
            />
            {" "}状態
          </div>
    ...
    <div className="p-6 lg:p-8 space-y-8 max-w-6xl mx-auto">
      ...
      <div>
        ...
        <p className="text-muted-foreground max-w-2xl">
          ...
          <TechTerm
            term="Rust製"
            definition="高速で安全なプログラミング言語で作られたシステム"
            example="C言語の速さとPythonの安全性を兼ね備えた言語"
          />
          インメモリポストストアです。
        </p>
      </div>
      ...
      <div className="rounded-xl border border-primary/20 bg-gradient-to-r from-primary/5 to-primary/10 p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          <div className="flex items-start gap-2">
            ...
            <div>
              <strong>瞬時の表示:</strong> タイムラインを開いた瞬間にフォロー中の投稿が表示されます（
              <TechTerm
                term="サブミリ秒 = 0.001秒未満"
                definition="瞬き1回(約100ms)の1/100以下の速さ"
                example="体感的には「遅延ゼロ」のレベル"
              />
              ）
            </div>
          </div>
          <div className="flex items-start gap-2">
            ...
            <div>
              <strong>リアルタイム反映:</strong> フォロー中の人が投稿した瞬間、あなたのタイムラインに即座に反映
            </div>
          </div>
          <div className="flex items-start gap-2">
            <Target className="h-4 w-4 text-emerald-400 mt-0.5 shrink-0" />
            <div>
              <strong>削除の即時反映:</strong> 投稿が削除されたら、すぐにタイムラインから消えます
            </div>
          </div>
          <div className="flex items-start gap-2">
            <Video className="h-4 w-4 text-purple-400 mt-0.5 shrink-0" />
            <div>
              <strong>動画の優先表示:</strong> 動画投稿が専用ストアで高速管理され、スムーズな視聴体験
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card/50 p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-4 text-center">
            ...
            <div className="text-sm font-semibold text-red-400">旧システム (Earlybird)</div>
            <div className="text-2xl font-bold font-mono mt-2 text-red-400">5-10ms</div>
            <div className="text-xs text-muted-foreground mt-1">ミリ秒単位</div>
          </div>
          ...
          <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
            <div className="text-3xl mb-2">🚀</div>
            <div className="text-sm font-semibold text-emerald-400">新システム (Thunder)</div>
            <div className="text-2xl font-bold font-mono mt-2 text-emerald-400">&lt;1ms</div>
            <div className="text-xs text-muted-foreground mt-1">サブミリ秒</div>
          </div>
        </div>
        <div className="mt-4 rounded-lg bg-primary/5 p-3 text-sm text-muted-foreground text-center">
          💡 <strong>例え:</strong> 旧システムが「徒歩」だとしたら、Thunderは「新幹線」レベルの速さ！
        </div>
      </div>
        ...
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="beginner">
            ...
            <AccordionContent className="text-sm text-muted-foreground space-y-2">
              <p>
                <strong>Thunderの役割:</strong> あなたがフォローしているアカウントの投稿を「高速倉庫」に保管して、瞬時に取り出せるようにするシステムです。
              </p>
              <p className="bg-primary/5 p-3 rounded-lg">
                💡 <strong>例え:</strong> コンビニの商品棚に例えると...
                <br />
                • 旧システム = 倉庫から取り寄せ（遅い）
                <br />
                • Thunder = 目の前の棚から取る（超高速）
              </p>
              <p>
                <strong>3種類の棚:</strong> 通常投稿📝、返信💬、動画🎥を別々の棚で管理することで、より速く正確に取り出せます。
              </p>
            </AccordionContent>
          </AccordionItem>
          ...
          <AccordionItem value="intermediate">
            ...
            <AccordionContent className="text-sm text-muted-foreground space-y-2">
              <p>
                <strong>Kafka:</strong> リアルタイムで情報を配信するシステム。投稿や削除のイベントをThunderに通知します。
              </p>
              <p className="bg-primary/5 p-3 rounded-lg">
                💡 <strong>例え:</strong> ニュース速報のようなもの。
                <br />
                • 誰かが投稿 → Kafkaがすぐ通知 → Thunderが倉庫に追加
                <br />
                • 誰かが削除 → Kafkaがすぐ通知 → Thunderが倉庫から削除
              </p>
              <p>
                <strong>インメモリストア:</strong> データをHDDやSSDではなく、コンピュータの高速メモリ(RAM)に保存します。これにより100倍以上高速なアクセスが可能です。
              </p>
              <p>
                <strong>TTL (Time To Live):</strong> 古い投稿は自動的に削除され、メモリを効率的に使います。
              </p>
            </AccordionContent>
          </AccordionItem>
          ...
          <AccordionItem value="advanced">
            ...
            <AccordionContent className="text-sm text-muted-foreground space-y-2">
              <p>
                <strong>3種分離の技術的理由:</strong>
              </p>
              <ul className="list-disc list-inside space-y-1 ml-2">
                <li><strong>Original Posts:</strong> 通常投稿は量が多いが、検索頻度は中程度</li>
                <li><strong>Replies + Reposts:</strong> 会話の流れを追うため、高頻度アクセスが必要</li>
                <li><strong>Video Posts:</strong> 動画は容量が大きく、専用最適化が必要</li>
              </ul>
              <p>
                各タイプで保持期間、インデックス戦略、メモリ配分を最適化することで、全体のパフォーマンスが向上します。
              </p>
              <p className="bg-primary/5 p-3 rounded-lg">
                💡 <strong>例え:</strong> スーパーの売り場配置に似ています。
                <br />
                • 日用品（通常投稿）= 広い棚
                <br />
                • レジ前商品（返信/リポスト）= すぐ取れる場所
                <br />
                • 冷凍食品（動画）= 専用冷蔵庫
              </p>
            </AccordionContent>
        ...
        <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-5">
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <li>🌐 
              <TechTerm
                term="In-Network候補"
                definition="フォロー中のアカウントからの投稿"
                example="あなたが既にフォローしている人の最新投稿"
              />
              をHome Mixerに提供
            </li>
          </ul>
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-6">
        ...
        <div className="space-y-2 text-sm text-muted-foreground">
          <p>
            <strong>旧システム (Earlybird) の問題:</strong>
          </p>
          <ul className="list-disc list-inside space-y-1 ml-2">
            <li>Javaベースで重い処理</li>
            <li>外部データベースへのアクセスが必要（ネットワーク遅延）</li>
            <li>削除がバッチ処理で遅延</li>
          </ul>
          <p className="mt-3">
            <strong>Thunder の革新:</strong>
          </p>
          <ul className="list-disc list-inside space-y-1 ml-2">
            <li>Rustで完全書き直し → 10倍以上の高速化</li>
            <li>完全インメモリ → データベースアクセス不要</li>
            <li>Kafkaでリアルタイム → 遅延ほぼゼロ</li>
            <li>動画専用ストア → スムーズな視聴体験</li>
          </ul>
          <p className="mt-3 bg-emerald-500/5 p-3 rounded-lg">
            🎯 <strong>結果:</strong> あなたのタイムラインは「待たされる」ことなく、瞬時に最新の投稿が表示されるようになりました。
          </p>
        </div>
      </div>
