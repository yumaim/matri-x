...
**⚠️ 「初速30分」の正確な理解:**
ソースコードの \`author_aggregate (real_time)\` は**ローリング30分ウィンドウ**です。
- ❌ 「投稿してから30分間」ではない
- ✅ 「直近30分間の著者全体のエンゲージメント」を計測
- つまり、**エンゲージメントが集中し始めたタイミングからの30分**が重要

**Grok導入後のフロー:**
投稿 → Grok品質評価 → 初期配信 → EG開始 → ★ここからの30分ウィンドウが本番 → 加速度上昇 → 配信拡大
